...
#Region "Load Form"
    Public Sub LoadForm()
        objForm = oApplication.Utilities.LoadForm(xml_SerialAssigment, frm_SerialAssigmnetImport)
...
#Region "Assign Serial Number"
    Private Function AssignSerialNo(ByVal aForm As SAPbouiCOM.Form) As Boolean
        ...
        Dim strItemCode, strSerialno, strWhs As String
        ...
        Try
            ...
            For intRow As Integer = 1 To oRowsMatrix.RowCount
                strItemCode = oApplication.Utilities.getMatrixValues(oRowsMatrix, "1", intRow)
                strWhs = oApplication.Utilities.getMatrixValues(oRowsMatrix, "3", intRow)
                dblSerialRequired = oApplication.Utilities.getDocumentQuantity(oApplication.Utilities.getMatrixValues(oRowsMatrix, "8", intRow))
                If dblSerialRequired > 0 Then
                    oTest.DoQuery("Select Count(*) from [@Z_OSRI] where U_Z_Available='Available' and  U_Z_ItemCode='" & strItemCode & "' and U_Z_Status='N'")
            ...
            For intRow As Integer = 1 To oRowsMatrix.RowCount
                ...
                If dblSerialRequired > 0 Then
                    ...
                    '    Return False
                    'End If
                    oTest.DoQuery("Select * from [@Z_OSRI] where  U_Z_Available='Available' and U_Z_ItemCode='" & strItemCode & "' and U_Z_Status='N'")
                    oSerialMatrix = aForm.Items.Item("3").Specific
                    For intLoop As Integer = 0 To dblSerialRequired - 1 ' oTest.RecordCount - 1
                        Dim stSerialNo As String = oTest.Fields.Item("U_Z_SerialNo").Value
                        'oApplication.Utilities.setEdittextvalue(aForm, "22", oTest.Fields.Item("U_Z_SerialNo").Value)
                        oApplication.Utilities.setEdittextvalue(aForm, "22", stSerialNo)
                        ...
                        aMatrix = aForm.Items.Item("5").Specific

                        For intRow1 As Integer = 1 To aMatrix.VisualRowCount
                            'If aMatrix.IsRowSelected(intRow1) Then
                            '    aForm.Items.Item("8").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                            '    Exit For
                            'End If
                            If oApplication.Utilities.getMatrixValues(aMatrix, "19", intRow1) = stSerialNo Then
                                aMatrix.Columns.Item("19").Cells.Item(intRow1).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                                aForm.Items.Item("8").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                                Exit For
                            End If
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_SerialAssigmnetImport Then
        ...
        For intRow As Integer = 1 To 1
            If aField1 <> "" Then
                ...
                If strCode = "" Then
                    ...
                    If orec.RecordCount <= 0 Then
                        oUserTable.UserFields.Fields.Item("U_Z_Available").Value = "Not Exists "
                    Else
                        oUserTable.UserFields.Fields.Item("U_Z_Available").Value = "Available"
                    End If
